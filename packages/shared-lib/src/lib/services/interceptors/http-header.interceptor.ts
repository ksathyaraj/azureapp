...
export class HttpHeaderInterceptor implements HttpInterceptor {
  ...
  constructor(private sessionService: SessionService,
      ...
      this.translateService.get('resources.common-httpinterceptorservice-error-unabletoconnectserver').subscribe((msg : string) => {
        this.unableToConnectServerMessage = msg;
      });
  }
  ...
  intercept(request: HttpRequest<unknown>, next: HttpHandler): Observable<HttpEvent<unknown>> {
    ...
    // onst accessToken=undefined;

    const headerConfig: any={
      'api_auth_key': env.api_auth_key,
      'x_apim_key': env.x_apim_key
    }
